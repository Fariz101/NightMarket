...
import EditSeller from "./edit";
import DeleteSeller from "./delete";
...
async function getSellers(
  ...
  try {
    const url = `${process.env.NEXT_PUBLIC_BASE_API_URL}/seller?page=${page}&limit=${limit}&search=${search}&sortBy=${sortBy}&sortOrder=${sortOrder}`;
...
export default async function SellersPage(prop: Props) {
  ...
  const { count: counts, data: sellers } = await getSellers(
    page,
    limit,
    search,
    sortBy,
    ...
    <div className="space-y-8 font-mono text-cyan-100 p-2">
      ...
      <div className="relative bg-[#031224]/80 backdrop-blur-xl border border-cyan-900/60 p-6 rounded-br-2xl rounded-tl-2xl border-l-4 border-l-orange-500 shadow-[0_0_30px_rgba(6,182,212,0.05)]">
        <div className="flex justify-between items-center text-[9px] tracking-widest text-cyan-500/50 font-black mb-1">
          <span>PAGE // SELLERS CRUD</span>
          <span>DATA COUNT: {counts} SELLERS</span>
        </div>
        ...
        <p className="text-xs text-cyan-500/70 mt-1 uppercase font-bold tracking-wide">
          view and manage seller data, including creating new sellers, editing existing ones, and deleting records as needed.
        </p>
        ...
        {/* 💡 FIXED GRID SYSTEM: Kalkulasi Akumulasi Pas 12 Kolom (1.5 + 2 + 1.5 + 1.5 + 1.5 + 2 + 2 = 12) */}
        <div className="hidden md:grid grid-cols-12 items-center bg-[#01070e] p-4 rounded-tl-md rounded-br-md text-[9px] font-black text-orange-400 uppercase tracking-[0.12em] border border-cyan-950 mt-6 mb-2 gap-4">
          ...
          <div className="col-span-1.5 md:col-span-1 flex items-center gap-1">
            <span>OWNER</span>
            <SortButton columnKey="owner" /> 
          </div>
          <div className="col-span-1.5 md:col-span-1.5 flex items-center gap-1">
            <span>PHONE</span>
              ...
              >
                ...
                <div className="col-span-1 md:col-span-2 min-w-0">
                  ...
                  <span className="font-medium text-cyan-100/90 block truncate w-full text-xs">
                    {seller.user?.email || "UNROUTED_NODE"}
                  </span>
                </div>
                ...
                {/* 4. OWNER NAME (col-span-1) */}
                <div className="col-span-1 md:col-span-1 min-w-0">
                  <small className="text-[8px] font-black text-orange-500/60 uppercase tracking-widest block md:hidden mb-0.5">
                    Owner Name
                  </small>
                  <span className="font-bold text-cyan-300 block truncate w-full uppercase tracking-wide">
                    {seller.owner || "-"}
                  </span>
                </div>

                {/* 5. PHONE (col-span-1.5) */}
                <div className="col-span-1 md:col-span-1.5 min-w-0">
                  <small className="text-[8px] font-black text-orange-500/60 uppercase tracking-widest block md:hidden mb-0.5">
                    Phone
                  </small>
                ...
                {/* 7. ACTIONS INTERACTION CONTROLS (col-span-2) */}
                <div className="col-span-1 md:col-span-2 flex items-center md:justify-end gap-2 shrink-0 text-right min-w-0 w-full mt-3 md:mt-0 pt-3 md:pt-0 border-t border-cyan-950/60 md:border-t-0">
                  <EditSeller selectedData={seller} />
                  <span className="text-cyan-950 hidden md:inline">/</span>
                  <DeleteSeller selectedData={seller} />
                </div>
              </div>
            ))}
